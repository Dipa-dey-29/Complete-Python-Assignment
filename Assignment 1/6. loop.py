...
def check_temperatures(temperatures, threshold):
    for temp in temperatures:
        if temp > threshold:
            print(f"Warning: Temperature {temp}°C exceeds the threshold!")
        elif temp <= threshold:
            print(f"Tempature {temp}°C is below to the threshold value")
...
def main():
    ...
    threshold = 40
    check_temperatures(temperatures, threshold)
...
def check_restock(products):
    for product, details in products.items():
        stock = details["stock"]
        min_required = details["min_required"]
        
        if stock < min_required:
            print(f"Alert: The product '{product}' is low on stock. Only {stock} items left. Minimum required is {min_required}. Please restock!")
